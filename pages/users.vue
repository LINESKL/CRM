<template>
  <div>
    <h3>Пользователи региона: {{ regionName }}</h3>
...
export default {
  ...
  methods: {
    async fetchUsers() {
      const encodedRegionName = encodeURIComponent(this.regionName);
      try {
        const response = await fetch(`http://127.0.0.1:8000/api/stats/region/${encodedRegionName}/`);
        const data = await response.json();
        this.users = data.last_activities;
